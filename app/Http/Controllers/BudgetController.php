...
class BudgetController extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($categorias as $cat) {
            ...
            $dadosCategorias[] = [
                ...
                'status' => $limite > 0 ? ($gastoValor > $limite ? 'excedido' : ($porcentagem >= 80 ? 'alerta' : 'ok')) : 'sem_orcamento',
            ];
        }
    ...
    {
        // ✅ CONVERTER VÍRGULAS PARA PONTOS ANTES DA VALIDAÇÃO
        $input = $request->all();
        
        // Converter valor_total
        ...
        if (!empty($input['categorias'])) {
            foreach ($input['categorias'] as $key => $cat) {
                if (!empty($cat['valor_limite'])) {
                    $input['categorias'][$key]['valor_limite'] = $this->parseFloat($cat['valor_limite']);
                } else {
                    $input['categorias'][$key]['valor_limite'] = 0;
                }
            }
        }
        
        // Agora validar com os dados convertidos
        ...
        return redirect()->route('budgets.index', ['mes' => $validated['mes'], 'ano' => $validated['ano']])
