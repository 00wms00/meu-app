{{-- Linha de uma despesa (usada em fixas e variáveis) --}}
<div class="px-5 py-3 flex items-center gap-3 hover:bg-gray-50 group" x-data="{ editing: false }">
    ...
    <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition shrink-0">
        <button @click="editing = !editing"
                class="p-1.5 rounded text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition" title="Editar">
        ...
        <form method="POST" action="{{ route('finance.expenses.destroy', $expense) }}">
            @csrf @method('DELETE')
            <button type="submit" onclick="return confirm('Remover esta despesa?')"
...
<div x-show="editing" x-cloak class="px-5 pb-4 bg-blue-50 border-t border-blue-100">
    <form method="POST" action="{{ route('finance.expenses.update', $expense) }}" class="grid grid-cols-2 gap-3 pt-3">
        @csrf @method('PUT')
